function [BER,SER] = KMelhores(n_symbols , n_iterations , bits_all , n_all , H_all , par)
...
for loop1=1:n_iterations    
    ...
    for ind_db = 1:length(par.SNRdB_list)
        ...
        for loop2 = 1:n_symbols
            ...
            %% KBEST MELHORES
            opcion = 2;   %1->ZF e 2->MMSE 
            yreal = H_real*s_real + w;
            [xhat_real,posicion] = KBEST(H_real,yreal,par,s_real,w,sigma,opcion); 
            %xhat_real = KBESTNew(H_real,yreal,par,s_real,w,sigma);  %OUTRO
            %xhat_real = KBESTNew2(H_real,yreal,par,s_real,w,sigma); %OUTRO->FEITO
